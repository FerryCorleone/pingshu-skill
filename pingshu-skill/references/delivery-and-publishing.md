...
```bash
node pingshu-skill/scripts/create_delivery_plan.mjs <audio_path> <delivery_plan.json>
```

上传并记录 manifest：

```bash
node pingshu-skill/scripts/publish_netease_cloud.mjs <audio_path> <manifest.json>
```

只演练命令、不真正上传：

```bash
node pingshu-skill/scripts/publish_netease_cloud.mjs <audio_path> <manifest.json> --dry-run
```
