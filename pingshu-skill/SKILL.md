...
用户要求“整集”“全集”“完整一期”“最后一期”“总决赛”或长音频时，不能只用官方简介、赛果表、百科和新闻稿拼成复盘。必须先做 `source_hunt`：

- 至少搜索官方平台、视频网站/长视频平台、短视频/社交平台、搜索引擎和多个二手 recap 线索；
- 优先找完整正片、官方纯享片段、字幕、ASR、分段 cut、时间线帖子和带时间戳的解说；
- 找不到芒果TV/腾讯/优酷/爱奇艺等官方可用材料，就继续查 B 站、YouTube、抖音、小红书、微博、Google/Bing、百度等线索；
- 把搜索过的平台、关键词、可用素材、不可访问原因和覆盖程度写进 `story_pack.source_hunt`；
- 只有拿到足够 `scene_trace` 才能生成“完整评书音频”。如果只有赛果、梗概或新闻通稿，只能明确产出“赛果/剧情复盘”，不能冒充整集还原。
...
标题使用原创回目式标题：两句并列，略有对仗，带人物/物件/动作/后果/反转，有一点章回体喜剧味。开头可短定场，例如“闲言少叙，书归正传。列位，今儿咱讲一回《标题》。”这一个报回目的短段后面应立即落一次醒木；背景和冲突从下一段开始讲，不要把故事正文塞进报回目段里。
...
```bash
node pingshu-skill/scripts/lint_pingshu_quality.mjs story_pack.json pingshu_script.json
```
...
生成 `performance_plan.json` 时，默认把 `voice.reference_voice.path_or_id` 设为 `pingshu-skill/assets/voice/default_storyteller_c06.wav`，并填入 manifest 里的 `reference_text`。除非用户明确指定自己的原创或已授权参考音频，否则不要重新设计默认音色，也不要覆盖内置音源。
...
node pingshu-skill/scripts/check_tts_readiness.mjs local-voxcpm2
```
...
- `seg-001` 只写短定场和回目标题；
- `seg-001.performance.sfx_after` 放 `["waking_block"]`；
- `seg-002` 才开始讲背景、人物和冲突；
- 如果需要把标题和正文放在同一个 segment，必须在 `performance_plan.events` 里把标题 `say` 单独拆出来，并把醒木挂在这个 `say` 之后。
...
```bash
node pingshu-skill/scripts/check_delivery_readiness.mjs netease-cloud
node pingshu-skill/scripts/setup_netease_cloud.mjs setup_manifest.json
node pingshu-skill/scripts/publish_netease_cloud.mjs audio.wav upload_manifest.json
```
