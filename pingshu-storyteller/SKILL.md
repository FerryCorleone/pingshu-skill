...
不要克隆或暗示模仿在世艺人的可识别声音。默认使用原创说书人 persona：中老年男声方向，清楚、略粗粝、带一点北方台口，单人表演，停顿讲究，笑里带刺。
...
如果设备低于 Qwen3-TTS 0.6B 的最低建议，优先走云端 API。

API TTS 要把风格/音色/导演提示放在服务商明确不会被朗读的位置。不要把自定义标签、括号提示或音色描述拼进正文。具体策略见 `references/tts-provider-matrix.md`。
