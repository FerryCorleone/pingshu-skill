...
不要把本地路线扩成模型大全。当前 Skill 对普通用户只暴露这两个档位：VoxCPM2 负责“效果优先”，Qwen3-TTS 0.6B 负责“配置较低也能试”。如果这两个暂时都不适合用户的机器，就先建议走云端 API。
...
- `assets/voice/default_storyteller_c06.wav`
- `assets/voice/manifest.json`

它是项目生成的原创参考声，不是真实艺人克隆。`render_tts_plan.mjs` 会默认把它写进 `performance_plan.voice.reference_voice`；本地 VoxCPM2 和 Qwen3-TTS 渲染器在没有用户自定义音源时会自动读取它。

本地路径先做设备检测：满足 VoxCPM2 最低配置就优先 VoxCPM2；不满足 VoxCPM2 但满足 Qwen3-TTS 0.6B，就使用 Qwen3-TTS；两个都不满足就建议 API。API 路径优先级：

1. **voice clone / voice enrollment**：用户同意把内置生成参考音频上传到所选服务商，或用户另有原创/已授权参考音频时优先。小米用 `mimo-v2.5-tts-voiceclone`；阿里 Qwen 用声音复刻创建 voice，再用 `qwen3-tts-vc-2026-01-22` 合成。
...
- Qwen：用户同意上传内置参考音频，或另有授权音频时，优先创建并复用同一个 Qwen voice clone id；否则 audition `qwen3-tts-vd` 或 `Arthur / 徐大爷` 这类系统声。
- 小米 MiMo：用户同意上传内置参考音频，或另有授权音频时，优先 `voiceclone`。`voicedesign` 只适合快速探索；长稿分段时要特别检查音色漂移和提示词泄漏。
